...
class _StatusGraphState extends State<StatusGraph> {
  Map<String, int> goalStatusCount = {
    'Pending': 0,
    'Completed': 0,
    'LateDone': 0,
  };
      ...
      FirebaseFirestore.instance
        ...
        querySnapshot.docs.forEach((doc) {
          setState(() {
            ...
            if (goalStatusCount.containsKey(status)) {
              goalStatusCount[status] = goalStatusCount[status]! + 1;
            }
          });
        });
      }).catchError((error) {
  ...
  List<PieChartSectionData> _getSections() {
    return goalStatusCount.entries.map((entry) {
